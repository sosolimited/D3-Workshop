...
var clientGraphView = (function(){
    ...
    // We need a custom index that starts at 0 for each map redraw, to delay the transitions only for new events.
	  var newMarkerIndex = 0;

    var newMarkers = markers.enter().append("circle")
      .attr("class", function(d,i) { return ( "marker "+ d.team ); })
      .attr("cx", function(d) { return projection( [ d.location[1], d.location[0] ] )[0] + wgwRandom(-10,10); })
      .attr("cy", function(d) { return projection( [ d.location[1], d.location[0] ] )[1] + wgwRandom(-10,10); });

    // If this is the first map draw, plop the markers in place without animating them.
    if ( geoEventMapFirstPull ) {

    	geoEventMapFirstPull = false;
    	newMarkers.attr("r", 5)
	      .style("opacity", 1);

    }
    // For all subsequent map draws, animate-in the new events.
    else {

    	newMarkers.attr("r", 50)
		    .style("opacity", 0)
		    // For every new circle, start with a large, pale circle, and shrink it to a small opaque one.
		    .transition()
      	.delay( function(){
      		var delayValue = newMarkerIndex * 1000;
